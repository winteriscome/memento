...
# Expose a safe wrapper so that code using python3 will still work when only 'python' exists
...
try:
  ...
  print(json.dumps({"tool": tool_name, "files": files, "summary": f"{tool_name}: {tool_response}"}))
except Exception:
...
import json, os
print(json.dumps({"external_session_id": os.environ["CLAUDE_SID"], "project": os.environ["MEMENTO_PROJECT"]}))
'
    )
...
try:
  d = json.load(sys.stdin)
  print(json.dumps({"external_session_id": sid, "content": d.get("summary", ""), "tool": d.get("tool", ""), "files": d.get("files", [])}))
except Exception:
...
try:
  d = json.load(sys.stdin)
  print(json.dumps({"external_session_id": os.environ["CLAUDE_SID"], "transcript_path": d.get("transcript_path", "")}))
except Exception:
...
')
    ...
    if [ -n "$EXTRACT_RESULT" ]; then
      echo "$EXTRACT_RESULT" | python3 - <<'PY'
import sys, json
...
')
  ...
  session-end)
    PAYLOAD=$(CLAUDE_SID="$CLAUDE_SID" python3 -c '
import json, os
print(json.dumps({"external_session_id": os.environ["CLAUDE_SID"], "outcome": "completed"}))
')
