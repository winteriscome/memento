...
本文档包含从 MVP 到远期愿景的完整设计。不同读者应关注不同章节：

| 读者身份         | 必读                                       | 选读                                | 跳过                     |
| ------------ | ---------------------------------------- | --------------------------------- | ---------------------- |
| **v0.1 实现者** | Ch1（哲学+定位）, Ch22.2（MVP 设计）, Ch22.4（验证实验） | Ch3.1（数据模型）, Ch4（生命周期）            | Ch5-22（v0.2+ 内容）       |
| **v0.2 架构师** | Ch1-5, Ch12-14（三轨/CQRS/状态机）, Ch17（工程约束）  | Ch15（API）, Ch18（冷启动）, Ch20（Agent） | Ch6-10（社交/联邦）, Ch21-22 |
| **全栈理解**     | 全文                                       | —                                 | —                      |
...
**核心赌注与风险**：本系统的全部设计建立在一个未经验证的假设上——**衰减 + 强化是否比纯向量搜索 + 时间排序更好？** v0.1 的唯一目标是验证这个假设。如果验证失败，v0.2 及后续所有章节均不应实施。第二到二十二章描述的是验证通过后的完整愿景，不是承诺交付的功能。
...
#### 20.5.1 接入层全景
    ...
    |  | (v0.1 全 Agent   |  | (v0.5 Claude Code|  | Schema       ||
    |  |  通用接入)        |  |  增强接入)        |  | (v0.5)       ||
    |  +--------+---------+  +--------+---------+  +------+-------+|
    |           |                     |                    |        |
    |           +---------------------+--------------------+        |
...
#### 20.5.2 统一 Agent API — 6 个工具

无论底层用 MCP、Function Calling 还是 CLI，Agent 看到的是同一组语义工具：
    ...
    engram_session_start(project_context?, task_description?)
      → { session_id, priming_memories: [Engram], project_conventions: [Engram] }

      触发时机: Agent 会话/任务开始时
      行为:
        1. 创建 session 记录 (关联 Agent DID + 项目路径)
        2. 自动 recall: 基于 project_context + task_description 检索相关记忆
        3. 返回 priming_memories: 最多 N 条高 strength 的相关记忆
           (注入 Agent context 作为"背景知识")
        4. 返回 project_conventions: 用户偏好、编码规范、架构决策等高 rigidity 记忆

    engram_session_end(session_id, outcome?, learnings[]?)
      → { captured_count, epoch_triggered }

      触发时机: Agent 会话/任务结束时
      行为:
        1. 如果 outcome 非空 → capture 为 episodic Engram (任务结果)
        2. 如果 learnings 非空 → 每条 capture 为独立 Engram
        3. 自动提取: 从 session 期间的 capture 记录中统计
           → 如果积累量达到 EpochTriggerPolicy 阈值 → 触发 mini-Epoch
        4. 关闭 session 记录
...
engram session end --outcome "fixed" --learnings "JWT token 过期时间需要配置化"
...
#### 20.6.1 会话三阶段
    ...
    ══════════════════════════════════════════════════════════
      ...
      engram_session_end(
        ...
        learnings = [
          "next-auth v5 的 middleware 配置方式变了",
          "该项目的 CI 在 PR 标题不符合 conventional commits 时会失败"
        ]
      )
           |
           v
      Engram:
        1. 自动 capture outcome + learnings
        2. 检查 EpochTriggerPolicy:
           - 累积 captures > threshold → 触发 mini-Epoch
           - token 消耗 > budget → 触发 full-Epoch
           - 否则 → 等待下次触发
...
### 22.1 五阶段演进
    ...
    (1-2 周)          (1-2 月)          (+ 2 月)          (+ 6 月)
      |                 |                 |                 |
      | capture/recall  | 三轨节律        | Fork/PR/Merge   | EFP 联邦协议
      | 惰性衰减        | CQRS            | 隐私系统         | 跨实例身份
      | export/import   | 六态状态机       | MCP Server      | 信任网络
      | CLI + 指令文件  | Delta Ledger    | Diplomat Agent  | 恶意实例隔离
      |                 | LLM 抽象化      | Project Vault   |
      |                 |                 |                 |
      核心假设验证:       验证通过后         个人记忆引擎       跨实例
      衰减+强化 vs       引入完整架构       + 有限共享         数据主权
      纯向量搜索
...
### 22.2 v0.1 "极简验证" — 一个 SQLite 文件的 MVP

> **设计原则：在第一行代码之前不做架构决策。**
> v0.1 的唯一目标是验证核心假设：**衰减 + 强化是否比纯向量搜索 + 时间排序更好？**
> 如果验证不通过，省下几个月工程量。如果通过，v0.2 再引入三轨/CQRS/状态机。
>
> **v0.1 是可丢弃原型，不是 v0.2 的地基。** v0.1 的单进程同步读写模型与 v0.2 的三轨异步架构之间没有渐进过渡的路径——如果核心假设被验证，v0.2 应当从零开始用正确架构重写。v0.1 的代码唯一要延续到 v0.2 的是**数据**（SQLite 中的记忆内容和元数据），不是代码。v0.1 的代码量应控制在 500-1000 行 Python 以内，确保丢弃时心理负担最小。
...
*   ~~三轨节律~~ → 单进程，无后台 worker
*   ~~CQRS~~ → 单 SQLite，读写同库
*   ~~六态状态机~~ → 只有 active / forgotten 两态
*   ~~Delta Ledger~~ → 衰减在 recall 时惰性计算
*   ~~两阶段提交~~ → SQLite 事务即可
*   ~~Epoch 批处理~~ → 无 Epoch，衰减和强化实时生效
*   ~~LLM 依赖~~ → v0.1 零 LLM 调用，抽象化推迟到 v0.2
...
| 能力                          | v0.1    | v0.2 | v0.5 | v1.0 |
| --------------------------- | ------- | ---- | ---- | ---- |
| capture / recall (CLI)      | **Y**   | Y    | Y    | Y    |
| 惰性衰减 + 再巩固                  | **Y**   | Y    | Y    | Y    |
| CLI + 指令文件接入                | **Y**   | Y    | Y    | Y    |
| export / import             | **Y**   | Y    | Y    | Y    |
| 三轨节律                        | -       | Y    | Y    | Y    |
| CQRS                        | -       | Y    | Y    | Y    |
| 六态状态机                       | -       | Y    | Y    | Y    |
| Delta Ledger                | -       | Y    | Y    | Y    |
...
| MCP Server                  | -       | -    | Y    | Y    |
| Fork / PR / Merge           | -       | -    | Y    | Y    |
| OperationLog 合并 (3.14-3.16) | -       | -    | Y    | Y    |
| 隐私系统 + 加密粉碎                 | -       | -    | Y    | Y    |
| Diplomat Agent (默认 OFF)     | -       | -    | Y    | Y    |
| Project Vault               | -       | -    | Y    | Y    |
| 联邦协议 EFP                    | -       | -    | -    | Y    |
| 跨实例身份                       | -       | -    | -    | Y    |
| ~~归属自动判定~~                  | **已移除** | -    | -    | -    |
...
*   Precision\@3 提升 > 15% **且** 过时记忆抑制率降低 > 50% → 引入完整三轨架构 (v0.2)
*   提升 < 15% 但方向正确 → 调参后重测一轮，或保持极简架构直接跳 v0.5
*   无提升或反而更差 → 重新审视整个设计哲学
...
11. **~~MVP 范围界定~~** → 四阶段演进路线：缸中之脑→社交→联邦（见第二十二章）
...
50. **~~v0.1→v0.2 断层未声明~~** → v0.1 明确声明为"可丢弃原型"，代码不延续到 v0.2，仅数据迁移（见 22.2 节）
